...
window.onload=function () {
    ...
    var get_search_by_url=decodeURI(location.search).slice(1).split('&');
    console.log(get_search_by_url);
    for (let i of get_search_by_url){
        let j=i.split('=');
        if (j[0]=='direction'){
            condition.direction=j[1]
        } else if (j[0]=='type'){
            condition.classify=j[1]
        }
    }
    //********************** header ********************************
    /* 点击跳到主页 */
    ...
    function show_dirction(directions){
        ...
        if (condition.direction!=''){
            for (let item of direction_content.querySelectorAll('li')){
                if (item.firstElementChild.innerText.indexOf(condition.direction)!=-1){
                    item.firstElementChild.className="selected";
                    previousElementSiblings(item,function (res) {
                        res.firstElementChild.className="";
                    });
                    nextElementSiblings(item,function (res) {
                        res.firstElementChild.className="";
                    });
                }
            }
    ...
    function get_courses(){
        let server=ip_config+'/course/courses/';
    ...
    // 根据condition筛选页码
    function get_pages(){
        let server=ip_config+'/course/pages/';
        ajax(server,condition,'GET',null,function (res) {
    ...
    function show_type(types){
        ...
        if (condition.classify!=''){
            for (let item of classify_content.querySelectorAll('li')){
                if (item.firstElementChild.innerText.indexOf(condition.classify)!=-1){
                    item.firstElementChild.className="selected";
                    previousElementSiblings(item,function (res) {
                        res.firstElementChild.className="";
                    });
